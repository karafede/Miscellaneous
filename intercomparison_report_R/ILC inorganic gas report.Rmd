...
bibliography: [ILC_GAS_PHASE_TECH_REPORT.bib]
# csl: sensors.csl
csl: nature.csl
link-citations: yes
header-includes:
...
Prova reference [@mukherjee_assessing_2017; @maurizio_kdjlkfdjgdkjlfdgdl_nodate]
...
ft <- flextable::flextable(Participants.df)
ft <- set_header_labels(ft, acronim = "Acronym", full_name = "Laboratory", country_name = "Country", code = "Code" )
ft <- theme_vanilla(ft)
ft <- color(ft, color = "black", part = "header")
ft <- flextable::width(ft, width = .7) # set width of all columns
...
# Loading query
instruments.df <- sqlFetch(InterDB, "instruments")[,c("code", "parameter", "analyser")]
instruments.df <- as.data.frame(instruments.df)
# disconnect
close(InterDB)

ft <- flextable::flextable(instruments.df)
ft <- merge_v(ft, j = "parameter")
ft <- theme_vanilla(ft)
ft <- flextable::width(ft, width = 1.20) # set width of all columns
ft <- fontsize(ft, part = "body", size = 8) 
ft <- fontsize(ft, part = "header", size = 10)
ft <- flextable::align( ft, align = "center", part = "all")
ft <- bg(ft, bg = "#E4C994", part = "header")
ft <- bg(ft, bg = "#C90000", part = "header")
ft <- color(ft, color = "white", part = "header")
ft <- set_header_labels(ft, code = "Code", parameter = "Pollutant", analyser = "Instruments")
ft <- autofit(ft)
ft
...
```{r, echo = FALSE, warning = FALSE, cache = FALSE, results = 'asis', out.width = "90%", message = FALSE, comment=FALSE}

Table3 <- read_csv("W:\\Intercomparisons\\development auto report R\\ILC Table 3 sequence.csv")
# Table3 <- read_excel("W:\\Intercomparisons\\development auto report R\\ILC Table 3 sequence.xlsx")
Table3 <- Table3[1:(ncol(Table3))]

T3 <- as.data.frame(Table3)

# T3$`start time`

ft <- flextable::regulartable(T3)
# fmt_datetime(ft, columns = vars(`start.time`))
    # time_style = 3) 
# ft <- set_formatter_type(x = ft, `start.time` = fmt_datetime( "%H:%M", ))
ft <- theme_vanilla(ft)
ft <- flextable::width(ft, width = .65) # set width of all columns
ft <- fontsize(ft, part = "body", size = 8)
ft <- fontsize(ft, part = "header", size = 8)
ft <- flextable::align( ft, align = "center", part = "all")
ft <-  bg(ft, bg = "#E4C994", part = "header")
autofit(ft)
ft
...
Table4$`b (nmol/mol)` <- round(Table4$`b (nmol/mol)`, digits = 0)
Table4[ Table4$Gas == "CO" , "a" ] <- round(Table4[ Table4$Gas == "CO" , "a" ], digits = 3)

# flextable set up
T4 <- as.data.frame(Table4)

ft <- flextable::regulartable(T4,
col_keys = c("Gas", "a", "b (nmol/mol)"))
# ft <- theme_vanilla(ft)
ft <- color(ft, color = "blue", part = "header")
ft <- flextable::width(ft, width = .99) # set width of all columns
ft <- fontsize(ft, part = "body", size = 9)
ft <- fontsize(ft, part = "header", size = 11)
ft <- flextable::align( ft, align = "center", part = "all")
ft <- bg(ft, bg = "#E4C994", part = "header")
ft <- colformat_num(
  x = ft, col_keys = "b (nmol/mol)",
  digits = 0)
autofit(ft)
...
```{r, fig.height=20, fig.width=35, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "100%", results = 'asis', message = FALSE, comment=FALSE}
...
Scores are given for each participant and each tested concentration level (run). The assessment criteria are presented as z’=±2 (blue line) and z’=±3 (red line). They represent the limits for the questionable and unsatisfactory results")
  ...
  fig <- ggplot(ISO13528_7_6_z.df %>% 
   ...
   labs(title = paste0("Parameter ", i), x = "", y = "z'-score") +
    scale_y_continuous(limit = c(-3.2, +3.2)) +
    scale_x_continuous(breaks = Tick.X) +
    # scale_x_continuos(name="run (1-j-q)", limits=c())
    geom_hline(yintercept = 2,  col = "blue", lty = 1) +
    geom_hline(yintercept = 3,  col = "red" , lty = 1) +
    geom_hline(yintercept = -2,  col = "blue", lty = 1) +
    geom_hline(yintercept = -3,  col = "red" , lty = 1) +
    # ggtitle(expression(paste("title position)"))) + 
    theme(plot.title = element_text(lineheight=1.8, face="bold", size = 36, hjust = 0.5))
  print(fig)
...
```{r, fig.height=10, fig.width=10, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}
...
## Create a connection with the Data Base
  ...
  # Loading query
  ISO13528_7_5_En.df <- sqlFetch(InterDB, "ISO 13528 (7_5)_En")[,c("code", "parameter", "En","run", "En_OK?")]
  
  # disconnect
  close(InterDB)
  
  tb <- ISO13528_7_5_En.df 
  
 
  qt <- filter(tb, En < -1 | En > 1)
  
  

  
ft <- flextable::regulartable(qt,  cwidth = 1.3, cheight = 0.4,
                             col_keys = c("code", "run", "En", "En_OK?" ))
# ft <- regulartable(ft)
ft <- theme_vanilla(ft)
ft <- color(ft, color = "black", part = "header")
ft <- flextable::width(ft, width = 1.0) # set width of all columns 
ft <- fontsize(ft, part = "body", size = 10)
ft <- fontsize(ft, part = "header", size = 11)
ft <- flextable::align( ft, align = "center", part = "all")
ft <- bg(ft, bg = "#E4C994", part = "header")
ft <- colformat_num(
   x = ft, col_keys = "En",
   digits = 1)
ft <- set_header_labels(ft, code = "Code", run = "Run", En = "En-score", `En_OK?` = "En evaluation")
ft  <- autofit(ft)
ft
  
# print(ft)
  
  # ft <- set_header_labels( ft, `En_OK?` = "En-score evaluation", code = "Code", run = "Run", En = "En score value")
  # ft <- bg(ft, bg = "#C90000", part = "header")
  # ft <- color(ft, color = "white", part = "header")
    # doc <- read_docx() %>% 
  # body_add_flextable(ft)
  # print(doc, target="word_style_FK.docx")
```
...
# pander::pander(Fig4)
# En’-score evaluations of CO measurements Scores are given for each participant and each tested concentration level (run).")
# print(Caption, quote = F)

```


```{r, fig.height=15, fig.width=30, fig.cap = "The caption: ID = {{Fig4}}", echo = FALSE, warning = FALSE, cache = FALSE, out.width = "90%", results = 'asis', message = FALSE, comment=FALSE}
...
for (i in unique(ISO13528_7_5_En.df$parameter)) {
  ...
  theme(plot.title = element_text(lineheight=.50, face="bold", size = 45, hjust = 0.5))
  
  Fig4 <- figs(name = paste0("Bias",i),caption = paste0(": Bias of participants's ",i))
  
  
  library(pander)
  pander(Fig4)
  pander::pander("En’-score evaluations of CO measurements Scores are given for each participant and each tested concentration level (run).")

Caption <-  paste0("Figure ", fig.nb,":  Reported values of ",i)
print(Caption, quote = F)
...
pander(Fig4, emphasize.strong.cols = 1, missing = "")
...
```{r, fig.height=10, fig.width=20, fig.cap = "The caption: ID = {{Fig4}}", echo = FALSE, warning = FALSE, cache = FALSE, out.width = "90%", results = 'asis', message = FALSE, comment=FALSE}
...
figNO2 <- ISO13528_7_5_En.df %>%
filter(parameter == "NO2")%>%
  filter()


for (i in unique(figNO2$parameter)) {
    # Caption
  # fig.nb <- grep(i, unique(ISO13528_7_5_En.df$parameter))
  
  figNO2$`run (1-j-q)` <- as.factor(figNO2$`run (1-j-q)`)
  
  fig <- ggplot(figNO2 %>% 
                  filter(parameter == i), 
                aes(x = `run (1-j-q)`, y = bias, fill = i)) + 
   ...
   labs(title = paste0("parameter ", i), x = "", y = "bias xi-X") +
     scale_y_continuous(limit = c()) +
      guides(fill=FALSE) +   # no legend
  #  scale_x_discrete(labels = Tick.X)
  theme(plot.title = element_text(lineheight=.30, face="bold", size = 15, hjust = 0.2))
  
  Fig4 <- figs(name = paste0("Bias",i),caption = paste0(": Bias of participants's ",i))
  pander::pander(Fig4)
  pander::pander("En’-score evaluations of CO measurements Scores are given for each participant and each tested concentration level (run). The     assessment criteria are presented as z’=±2 (blue line) and z’=±3 (red line). They represent the limits for the questionable and     unsatisfactory results")
 browser()
  print(fig)
...
# Loading query
Measured_Data.df <- sqlFetch(InterDB, "Measured Data")[,c("parameter", "run (1-j-q)", "lab (1-i-p)", "yijk")]
labs <- sqlFetch(InterDB, "labs")[ ,c("acronim", "code")]
names(labs)[names(labs) == 'acronim'] <- "lab (1-i-p)"

Measured_Data.df <- Measured_Data.df %>%
  left_join(labs, by = "lab (1-i-p)")

# disconnect
close(InterDB)

ft <- Measured_Data.df[,c("parameter", "run (1-j-q)", "lab (1-i-p)", "yijk", "code")]

ft_filter <- ft %>%
  dplyr::filter(parameter %in% c("NO2", "NO")) %>%
  dplyr::filter(`run (1-j-q)` > 0) 
  
ft_filter <- ft_filter[order(ft_filter$code), ]

ft_filter <- ft_filter %>%
  group_by(parameter,
           `run (1-j-q)`,
           code) %>%
summarise(mean = mean(yijk))

CONV_eff <- function(Index, DF) {
   return(abs((DF[DF$parameter == "NO2" & DF$`run (1-j-q)` == Index, "mean"] -
            DF[DF$parameter == "NO2" & DF$`run (1-j-q)` == Index -1, "mean"])/
     (DF[DF$parameter == "NO" & DF$`run (1-j-q)` == Index, "mean"] -
            DF[DF$parameter == "NO" & DF$`run (1-j-q)` == Index - 1, "mean"])*100))
 }
 CONV_df <- data.frame(sapply(unique(ft_filter$code), function(Lab){
   ft_code <- ft_filter %>% 
     dplyr::filter(code == Lab)
   DF_CONV <- unlist(sapply(seq(from = 2, to = length(ft_code$`run (1-j-q)`), by = 2), 
                            CONV_eff, DF = ft_code))
   return(DF_CONV)
 } ))
 names(CONV_df)     <- unique(ft_filter$code)
 CONV_df$Runs <- paste0("Run", seq(from = 2, to = length(unique(ft_filter$`run (1-j-q)`)), by = 2))
 
 ft <- flextable(CONV_df[, c(which(names(CONV_df) == "Runs"), grep(pattern = "Runs", x = names(CONV_df), invert = TRUE))])
...
ft <- theme_vanilla(ft)
ft <- color(ft, color = "black", part = "header")
ft <- flextable::width(ft, width = .95) # set width of all columns 
ft <- fontsize(ft, part = "body", size = 10)
ft <- fontsize(ft, part = "header", size = 11)
ft <- flextable::align( ft, align = "center", part = "all")
ft <- bg(ft, bg = "#E4C994", part = "header")
ft <- colformat_num(x = ft, col_keys = "A", digits = 1)
ft <- colformat_num(x = ft, col_keys = "B", digits = 1)
ft <- colformat_num(x = ft, col_keys = "C", digits = 1)
ft <- colformat_num(x = ft, col_keys = "D", digits = 1)
ft <- colformat_num(x = ft, col_keys = "E", digits = 1)
ft <- colformat_num(x = ft, col_keys = "F", digits = 1)
ft <- colformat_num(x = ft, col_keys = "G", digits = 1)
ft <- colformat_num(x = ft, col_keys = "H", digits = 1)
ft <- add_header(ft, D = "Laboratory code", top = TRUE )
# ft <- colformat_num(x = ft, col_keys = "I", digits = 1)
# ft <- colformat_num(x = ft, col_keys = "K", digits = 1)
...
InterDB <- RODBC::odbcConnectAccess2007(InterDB.path)
# Loading query
ISO13528_flags.df <- sqlFetch(InterDB, "ISO 13528 flags")[,c("code", "parameter", "flag", "run (1-j-q)", "yref,j")]
# disconnect
close(InterDB)

ft <- ISO13528_flags.df[,c("code", "parameter", "flag","run (1-j-q)", "yref,j")]%>%
spread(code, flag)
ft <- as.data.frame(ft)

ft <- flextable::regulartable(ft)
ft <- colformat_num( x = ft, col_keys = "run (1-j-q)", digits = 0)
ft <- colformat_num( x = ft, col_keys = "yref,j", digits = 2)
# ft<-bg(ft, i = ~ A < 2, j = 4:12, bg = "green")

ft <- merge_v(ft, j = "parameter")
ft <- theme_vanilla(ft)
ft <- add_header(ft, D = "Categories", top = TRUE )
# ft <- color(ft, color = "black", part = "header")
# ft <- flextable::width(ft, width = .95) # set width of all columns 
# ft <- fontsize(ft, part = "body", size = 10)
# ft <- fontsize(ft, part = "header", size = 11)
...
## Create a connection with the Data Base
InterDB.path <- file.path("E:\\BKUP\\Desktop\\LCE_Template.GAS-PHASE.accdb")
# InterDB.path <- file.choose("E:\\BKUP\\Desktop\\LCE_Template.GAS-PHASE.accdb")
InterDB <- RODBC::odbcConnectAccess2007(InterDB.path)
 ## Import the table/query

assigned_values.df <- sqlFetch(InterDB, "Assigned values")[,c("run", "MinOfunit", "yref,j", "u_yrefj", "Uyrefj", "_x*", "_s*", "p")]
...
assigned_values.df <- assigned_values.df[order(assigned_values.df$run),]
assigned_values.df <- as.data.frame(assigned_values.df)

# ht <- huxtable::huxtable(assigned_values.df)

# ft <- theme_vanilla(ft)


ft <- flextable::regulartable(assigned_values.df)
...
ft <- flextable::align( ft, align = "center", part = "all")
ft <-  bg(ft, bg = "#E4C994", part = "header")
ft <- colformat_num(x = ft, col_keys = "_s*", digits = 2)
ft <- colformat_num(x = ft, col_keys = "_x*", digits = 2)
ft <- colformat_num(x = ft, col_keys = "_s*", digits = 2)
ft  <- autofit(ft)
ft
# doc <- read_docx() %>% 
...
ft <- flextable::regulartable(Table10)
...
```{r, fig.height=10, fig.width=25, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "90%", results = 'asis', message = FALSE, comment=FALSE}
...
for (i in unique(Measured_Data.df$parameter)) {
  ...
  # Caption
  fig.nb <- fig.nb + 1
  # Caption <-  paste0("Figure ", fig.nb,":  Reported values of ",i," concentration ",j," ")
  # print(Caption, quote = F)
  ...
  theme_bw() +
   geom_point()  +
   geom_errorbar(filtered_data, mapping = aes(x="",
                ymin = mean_yijk-mean_Uyij , 
                ymax=mean_yijk+mean_Uyij),
                na.rm=TRUE, 
                position="dodge", 
                width=0.3, # width end of error bar
                size=1.0,) +# width line error bar
  labs(title = paste0("Parameter ", i, " conc level: ", j), x = "", y = paste0(i," ", unique(filtered_data$unit_yijk))) +
  facet_grid(. ~ filtered_data$code) +
  geom_text_repel( aes(label = filtered_data$mean_yijk), size = 12, show.legend = FALSE) +
  geom_hline(aes(yintercept = filtered_data[filtered_data$code == "G" , ]$ref ),  col = "red", lty = 30) +

      # no legend
  guides(fill=FALSE) +   
  # theme(plot.title = element_text(lineheight=1.1, face="bold", size = 35, hjust = 0.5) 
          theme(
plot.title = element_text(lineheight=10, color="red", size=30, face="bold", hjust = 0.5),
...
#  run different to zero level (3 values)
if (nrow(vals_yijk) > 1) {
  stats <- Measured_Data.df %>% 
    dplyr::filter(`run (1-j-q)` == j,
           parameter == i) %>%
   dplyr::select(code,
          yijk,
          Uyij,
...
values <- row.names(Table)
rownames(Table) <- NULL
Table <- cbind(values,Table)
}

#  run equal to zero level (1 value)
...
panderOptions('table.alignment.default', function(df) ifelse(sapply(df, is.numeric), 'right', 'left')) # right alignment for numeric, left otherwise
panderOptions('knitr.auto.asis', FALSE)
pander(Table, emphasize.strong.cols = 1, missing = "")
...
```{r, fig.height=8, fig.width=14, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}

Fig45 <- figs(name="rRCO'", "The R and r of SO2 standard measurement method")
pander::pander(Fig45)

```

```{r, fig.height=7, fig.width=14, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "80%", results = 'asis', message = FALSE, comment=FALSE}
...
# ft <- flextable::regulartable(TabrR)
# # ft <- regulartable(ft)
# ft <- theme_vanilla(ft)
# ft <- color(ft, color = "black", part = "header")
# ft <- flextable::width(ft, width = .95) # set width of all columns 
...
fig <- ggplot(data = Tab.for.fig2,
  ...
  # no legend
  guides(fill=FALSE) +
  theme(plot.title = element_text(lineheight=.9, face="bold", size = 70, hjust = 0.5))
print(fig)
...
``` {r, fig.height=9, fig.width=15, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}

Fig46 <- figs(name="rRCO'", "The R and r of CO standard measurement method")
pander::pander(Fig46)
...
fig <- ggplot(data = Tab.for.fig2,
  ...
  aes(x=`mj=yj(19)`, y=r1, colour=rR)) +
  geom_line(size = 1) +
  xlab("CO concentration (nmol/mol)") +
  ylab("r&R (nmol/mol)") +
  theme_bw(base_size = 25)
  # no legend
  guides(fill=FALSE) +
  theme(plot.title = element_text(lineheight=.8, face="bold", size = 70, hjust = 0.5))
print(fig)
...
Fig47 <- figs(name="rRCO'", "The R and r of O3 standard measurement method")
pander::pander(Fig47)
...
fig <- ggplot(data = Tab.for.fig2,
  ...
  aes(x=`mj=yj(19)`, y=r1, colour=rR)) +
  geom_line(size = 1) +
  xlab("O3 concentration (nmol/mol)") +
  ylab("r&R (nmol/mol)") +
  theme_bw(base_size = 25)
  # no legend
  guides(fill=FALSE) +
...
Fig48 <- figs(name="rRCO'", "The R and r of NO standard measurement method")
pander::pander(Fig48)
...
fig <- ggplot(data = Tab.for.fig2,
  ...
  aes(x=`mj=yj(19)`, y=r1, colour=rR)) +
  geom_line(size = 1) +
  xlab("NO concentration (nmol/mol)") +
  ylab("r&R (nmol/mol)") +
  theme_bw(base_size = 25)
  # no legend
  guides(fill=FALSE) +
...
Fig49 <- figs(name="rRCO'", "The R and r of NO2 standard measurement method")
pander::pander(Fig49)
...
fig <- ggplot(data = Tab.for.fig2,
  ...
  aes(x=`mj=yj(19)`, y=r1, colour=rR)) +
  geom_line(size = 1) +
  xlab("NO2 concentration (nmol/mol)") +
  ylab("r&R (nmol/mol)") +
  theme_bw(base_size = 25)
  # no legend
  guides(fill=FALSE) +
...
## Create a connection with the Data Base
  ...
  # Loading query
  ISO13528_7_6_z.df <- sqlFetch(InterDB, "ISO 13528 (7_6)_z")[,c("code", "parameter", "z","run", "z_OK?")]
  
  # disconnect
  close(InterDB)
  
  tb <- ISO13528_7_6_z.df 
    quest <- filter(tb, z > 2 | z < -2)
  

ft <- flextable::regulartable(
  head(quest),
  cwidth = 1.3, cheight = 0.25,
  col_keys = c("code", "run", "z", "z_OK?" ))

ft <- set_header_labels( ft, `z_OK?` = "z'-score evaluation", code = "Code", run = "Run", z = "z'score value")
ft <- bg(ft, bg = "#C90000", part = "header")
# ft <- color(ft, color = "white", part = "header")
