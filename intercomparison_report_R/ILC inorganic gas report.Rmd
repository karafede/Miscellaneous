...
ft <- width(ft, width = .85) # set width of all columns
...
ft <- flextable(instruments.df)
ft <- merge_v(ft, j = "parameter")
ft <- theme_vanilla(ft)
ft <- width(ft, width = 1.20) # set width of all columns
...
# T3$`start time`
ft <- regulartable(T3)
  # fmt_datetime(T3, columns = vars(ft),
    # time_style = 3) 
# ft <- set_formatter_type(x = ft, `start.time` = fmt_datetime( "%H:%M", ))
ft <- theme_vanilla(ft)
ft <- width(ft, width = .55) # set width of all columns
ft <- fontsize(ft, part = "body", size = 8)
...
ft <- width(ft, width = .99) # set width of all columns
...
ft <- ISO13528_flags.df[,c("code", "parameter", "flag","run (1-j-q)", "yref,j")] %>% 
  spread(code,flag)
ft$`run (1-j-q)` <- round(ft$`run (1-j-q)`, digits = 0)
ft$`yref,j` <- round(ft$`yref,j`, digits = 2)
ft <- flextable(ft)
ft <- fontsize(ft, size = 14, part = "all")

# 
# ft <- ISO13528_flags.df[c("code", "parameter", "flag"),]
# ft <- flextable(ft)
# ft <- set_header_labels( ft, code )
# ft <- border_remove( ft )
# 
...
# # row name
# run <- with(ft, "run (1-j-q)" < 2 )
# # column name
# code <- c(code)
# 
# ft <- color(ft, i = run, j = code, color="red") 
# 
# 
# ft <- color(ft, i = 1, j = ,c(("run")), color = "green")
...
ft <- theme_vanilla(ft)
ft <- color(ft, color = "black", part = "header")
ft <- width(ft, width = .95) # set width of all columns
ft <- fontsize(ft, part = "body", size = 10)
ft <- fontsize(ft, part = "header", size = 11)
ft <- align( ft, align = "center", part = "all")
ft <-  bg(ft, bg = "#E4C994", part = "header")
