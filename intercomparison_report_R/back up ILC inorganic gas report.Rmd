...
output:
  # pdf_document:
    ...
    # fig_caption: true
  word_document: 
  reference_docx: Template_MAURIZIO_inter.docx
...
participant <- file.path("labo:\\Intercomparisons\\development auto report R\\ILC acknowledgment.xlsx")
participant <- readxl::read_excel(participant)
...
```{r 1, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE }
# Caption
...
The test mixtures were prepared by the dilution of gases from cylinders containing high concentrations of NO, SO2 or CO using thermal mass flow controllers [8]. O3 was added using an ozone generator and NO2 was produced applying the gas phase titration method [19] in a condition of NO excess.
...
```{r Table 3, fig.height=16, fig.width=12, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}

Table3 <- read_csv("W:\\Intercomparisons\\development auto report R\\ILC Table 3 sequence.csv")

# Table3 <- read.csv("W:\\Intercomparisons\\development auto report R\\ILC Table 3 sequence.csv")

Table3 <- as.data.frame(Table3)
# Table3 <- Table3[!is.na(Table3$X), ]


# tableOutput(excel_sheets(W:\\Intercomparisons\\development auto report R\\ILC Table 3 sequence.xlsm))	
...
```{r Table 4, fig.height=16, fig.width=12, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}

Table4 <- read_excel("W:\\Intercomparisons\\development auto report R\\ILC table 4 aquila parameter.xlsm")
...
# tableOutput(excel_sheets(W:\\Intercomparisons\\development auto report R\\ILC table 4 aquila parameter.xlsm))	
...
```{r fig3, fig.height=9, fig.width=12, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}
...
``````{r fig6, fig.height=9, fig.width=12, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}
...
InterDB <- RODBC::odbcConnectAccess2007(InterDB.path)
 ## Import the table/query
# qry <- “SELECT * FROM `ISO 13528(7_5)_En$`”
 # assigned values <- sqlQuery(con, qry)
 # str("code", "parameter", "En")
...
for (i in unique(ISO13528_7_5_En.df$parameter)) {
  ...
  Caption <-  paste0("Figure ", fig.nb,": En’-score evaluations of CO measurements 
Scores are given for each participant and each tested concentration level (run). The assessment criteria are presented as z’=±2 (blue line) and z’=±3 (red line). They represent the limits for the questionable and unsatisfactory results")
  print(Caption, quote = F)
  ...
  fig <- ggplot(ISO13528_7_5_En.df %>% 
   ...
   labs(title = paste0("parameter ", i), x = "", y = "En-score") +
...
# Caption
Caption <-  "8: Verification of assigned values (X)."
set.caption(Caption)
...
pander(assigned_values.df, emphasize.strong.cols = 1, missing = "")
...
```{r Table 9, fig.height=16, fig.width=12, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}

Table9 <- read_excel ("W:\\Intercomparisons\\development auto report R\\ILC Table 9 flags summary.xlsx")

# add_header_lines(x, values = character(0), top = TRUE)

Table9 <- as.data.frame(Table9)
Table9 <- Table9[!is.na(Table9$ILC), ]
# Table9 <- Table9[c(1:9)]
...
Table9 <- set_header( Table9, "ILC", "Site", "1", "2", "3", "4", "5", "6", "7" )


# Table9 <- italic(Table9, j = 1)
# Table9 <- bg(Table9, bg = "#C90000", part = "header")
# Table9 <- color(Table9, color = "white", part = "header")
# Table9 <- color(Table9, ~ Site > 3.5, ~ Site, color = "red")
# Table9 <- bold(Table9, ~ Site > 3.5, ~ Site, bold = TRUE)
...
```{r Table 10, fig.height=16, fig.width=12, echo = FALSE, warning = FALSE, cache = FALSE, out.width = "70%", results = 'asis', message = FALSE, comment=FALSE}

Table10 <- read_excel ("W:\\Intercomparisons\\development auto report R\\ILC Table 10 zscore summary.xlsx")

# Table10 <- read.csv("W:\\Intercomparisons\\development auto report R\\ILC Table 10 zscore summary.xlsx")

Table10 <- as.data.frame(Table10)
# Table10 <- Table10[!is.na(Table10$X), ]


# tableOutput(excel_sheets(W:\\Intercomparisons\\development auto report R\\ILC Table 10 zscore summary.xlsx))	

Table10$start.time

# Caption
Caption <-  "10: z'-score summary."
set.caption(Caption)
